...
```
esp32_image/             # ESP32 文件系统镜像（此目录内容 = 推流到板子的全部文件）
  main.py                #   主程序：硬件初始化、主循环（1Hz时钟 + 30min天气轮询）、WDT
  common/net_weather.py  #   网络模块：WiFi、校园网Portal认证、心知天气API
  vga1_8x16.py           #   8x16 点阵字体（ASCII 0x20-0x7f）
  vga2_16x32.py          #   16x32 点阵字体（0-255 全字符集）
  *.bin                  #   天气图标 RGB565 二进制（39个，0.bin ~ 38.bin）
wether_UI/               # 天气图标 XBM 源文件（32x32 单色，22个）
xbm2bin.py               # 构建工具：wether_UI/*.xbm → esp32_image/*.bin
test/                    # 开发调试脚本（非生产代码）
...
### 主循环策略
`main.py` 采用单调非阻塞轮询而非 asyncio：通过 `time.ticks_diff()` 实现两个逻辑协程 —— 1Hz 时钟刷新和 30 分钟天气刷新。WDT 超时 8 秒，每次循环喂狗。
...
- 天气图标预分配 32x32x2 字节 `bytearray` 池，防止碎片化
- 每次网络操作后调用 `gc.collect()`
- API 响应缓冲区 128 字节上限，超出后滑动窗口截断

### 天气图标构建
`xbm2bin.py` 将 `wether_UI/*.xbm` 转为 `esp32_image/{code}.bin`。心知天气全量 39 个 code (0-38) 映射见脚本内 `icon_map`。颜色：前景 0xFD20（橙黄），背景 0x0000（纯黑）。图标严格 32x32 像素（128 字节 XBM 数据）。

## ESP32 串口通信（重要）

**ESP32-C3 设备**: COM8, vid:303a pid:1001（内置 USB-Serial-JTAG）

**关键限制**: Git Bash (MSYS2) 环境下 Python 的 `pyserial` 对 Windows COM 口存在底层阻塞 bug。**所有 mpremote 命令必须通过 `.bat` 批处理文件执行**，bash 会自动将 `.bat` 委托给 Windows 原生命令行，绕过串口阻塞。
...
# 推送 esp32_image 全部文件到板子
echo "@echo off && for /r esp32_image %f in (*) do python -m mpremote connect COM8 cp %f :%~nxf" > tmp.bat && ./tmp.bat
...
1. 首次烧录：用 `esptool` 写入 `firmware_4MiB.bin`（MicroPython 4MB Flash，含 `gc9a01` 驱动）
2. 日常更新：通过上述 `.bat` 包装的 mpremote 将 `esp32_image/` 下文件推流至 ESP32
